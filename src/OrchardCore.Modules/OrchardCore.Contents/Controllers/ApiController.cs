    ...
    public class ApiController : Controller
    {
        ...
        public async Task<IActionResult> Post(ContentItem newContentItem, bool draft = false)
        {
            ...
            if (contentItem == null)
            {
                ...
                if (!await _authorizationService.AuthorizeAsync(User, Permissions.ViewContent))
                {
                    return Unauthorized();
                }

                if (System.String.IsNullOrEmpty(newContentItem.ContentItemId))
                {
                    var newContentItemWithId = await _contentManager.NewAsync(newContentItem.ContentType);
                    newContentItem.ContentItemId = newContentItemWithId.ContentItemId;
                }

                // Set the current user as the owner to check for ownership permissions on creation
                newContentItem.Owner = "admin";
